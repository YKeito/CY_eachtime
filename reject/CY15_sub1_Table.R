...
cname <- cname[order(cname, decreasing = F)]
...
#######read_file#############################################################################################################################################################################
filename <- list.files("~/Nakano_RNAseq/network_analysis/base/Subcluster_Information/sub1_info/CY15", pattern=".csv", full.names = T)
temp2 <- rep(c("12h", "1h","24h", "3h"), each = 5)
temp3 <- rep(c("sub1", "Botrytis_cinerea", "BTH", "MeJA", "PstDC3000"), times = 4)
object_name <- c()
object_name_all <- c()
test <- c()
n <- 1
...
CY15_Table <- CY15_Table[, order(colnames(CY15_Table), decreasing = F)]

write.table(CY15_Table, file = "~/Nakano_RNAseq/network_analysis/eachCY_Table/CY15_Table.txt", append=F, quote = F, sep = "\t", row.names = F)
